---
# Display name
title: Akhahenda k. Whitney
# Is this the primary user of the site?
superuser: true

# Role/position
role: PhD Student in Maritime AI & Cybersecurity

# Organizations/Affiliations
organizations:
  - name: National Korea Maritime and Ocean University
    url: 'https://www.kmou.ac.kr/'
  - name: Jomo Kenyatta University of Agriculture and Technology
    url: 'https://www.jkuat.ac.ke/'
    
# Bio (shown in the about widget)
Bio: A Kenyan researcher exploring the intersection of AI, maritime emissions, and cybersecurity in Busan, South Korea.

Interests:
  - Ship Emissions Modeling
  - Maritime Big Data (AIS & IHS Data)
  - AI, Machine Learning & Data Analytics
  - Sustainable Maritime/Shipping Industry: IMO (MARPOL) Regulations

Education:
  courses:
    - course: PhD in Maritime AI & Cybersecurity 
      institution: National Korea Maritime and Ocean University
      location: Busan, South Korea
      period: March, 2025 — Present
    - course: MSc in Maritime AI & Cybersecurity
      institution: National Korea Maritime and Ocean University
      location: Busan, South Korea
      period: March, 2023 — February, 2025
    - course: BSc in Marine Engineering
      institution: Jomo Kenyatta University of Agriculture and Technology
      location: Juja, Kenya
      period: September, 2015 — December, 2020


# Social/Academic Networking
social:
  - icon: envelope
    icon_pack: fas
    link: 'mailto:akhahenda@g.kmou.ac.kr'
  - icon: github
    icon_pack: fab
    link: https://github.com/akhahenda
  - icon: instagram
    icon_pack: fab
    link: https://instagram.com/whitneykhaye
  - icon: youtube
    icon_pack: fab
    link: https://youtube.com/www.youtube.com/@whitneyakhahenda

# Enter email to display Gravatar (optional)
email: "akhahenda@g.kmou.ac.kr"
---

Hello! I am a Kenyan researcher based in South Korea. I am currently a PhD student at ** National Korea Maritime and Ocean University**, focusing on how Artificial Intelligence can improve maritime sustainability and security. 

On this site, I share summaries of my research, technical projects from my GitHub, and insights into my journey as a researcher in the maritime field.
